...
static const struct dev_entry devs_ch347_spi[] = {
    {0x1A86, 0x55DB, OK, "QinHeng Electronics",
     "USB To UART+SPI+I2C"}, /* CH347T */
    {0x1A86, 0x55DE, OK, "QinHeng Electronics",
     "USB To UART+SPI+I2C"}, /* CH347F */
    {0}};
#ifndef _WIN32
...
static const struct device_speeds spispeeds[] = {
    {"60M", 0},    {"30M", 1},    {"15M", 2},     {"7.5M", 3}, {"3.75M", 4},
    {"1.875M", 5}, {"937.5K", 6}, {"468.75K", 7}, {NULL, 0}};
...
static int ch347_cs_control(struct ch347_spi_data *ch347_data, uint8_t cs1,
                            uint8_t cs2) {
  uint8_t cmd[13] = {[0] = CH347_CMD_SPI_CS_CTRL,
  ...
  if (!CH347WriteData(ugIndex, cmd, &transferred) ||
      transferred != sizeof(cmd)) {
    msg_perr("Could not change CS!\n");
    return -1;
  }
#else
  int32_t ret = libusb_bulk_transfer(ch347_data->handle, WRITE_EP, cmd,
                                     sizeof(cmd), NULL, 1000);
  if (ret < 0) {
    msg_perr("Could not change CS!\n");
    return -1;
  }
...
static int ch347_write(struct ch347_spi_data *ch347_data, unsigned int writecnt,
  ...
  while (bytes_written < writecnt) {
    ...
    if (!CH347WriteData(ugIndex, buffer, &transferred) ||
        transferred != (unsigned long int)packet_len) {
      msg_perr("Could not send write command\n");
      return -1;
    }
    transferred = sizeof(resp_buf);
    if (!CH347ReadData(ugIndex, resp_buf, &transferred) ||
        transferred != sizeof(resp_buf)) {
    ...
    ret = libusb_bulk_transfer(ch347_data->handle, READ_EP, resp_buf,
                               sizeof(resp_buf), NULL, 1000);
    if (ret < 0) {
      msg_perr("Could not receive write command response\n");
      return -1;
    }
...
static int ch347_read(struct ch347_spi_data *ch347_data, unsigned int readcnt,
                      uint8_t *readarr) {
  ...
  uint8_t buffer[CH347_PACKET_SIZE] = {0};
  uint8_t command_buf[7] = {[0] = CH347_CMD_SPI_IN,
                            [1] = 4,
                            [2] = 0,
                            [3] = readcnt & 0xFF,
                            [4] = (readcnt & 0xFF00) >> 8,
  ...
  if (!CH347WriteData(ugIndex, command_buf, &transferred) ||
      transferred != sizeof(command_buf)) {
    msg_perr("Could not send read command\n");
    return -1;
  }
#else
  ret = libusb_bulk_transfer(ch347_data->handle, WRITE_EP, command_buf,
                             sizeof(command_buf), &transferred, 1000);
...
#else
    ret = libusb_bulk_transfer(ch347_data->handle, READ_EP, buffer,
                               CH347_PACKET_SIZE, &transferred, 1000);
    if (ret < 0) {
      msg_perr("Could not read data\n");
      return -1;
    }
...
static int ch347_spi_send_command(const struct flashctx *flash,
  ...
  ch347_cs_control(ch347_data, CH347_CS_ASSERT | CH347_CS_CHANGE,
                   CH347_CS_IGNORE);
  if (writecnt) {
  ...
  ch347_cs_control(ch347_data, CH347_CS_DEASSERT | CH347_CS_CHANGE,
                   CH347_CS_IGNORE);

  return 0;
}

static int32_t ch347_spi_config(struct ch347_spi_data *ch347_data,
                                uint8_t divisor) {
  int32_t ret;
  uint8_t buff[29] = {[0] = CH347_CMD_SPI_SET_CFG,
                      ...
                      [24] = 0};
#ifdef _WIN32
  long unsigned int transferred = sizeof(buff);
  ...
  ret = libusb_bulk_transfer(ch347_data->handle, WRITE_EP, buff, sizeof(buff),
                             NULL, 1000);
  if (ret < 0) {
    msg_perr("Could not configure SPI interface\n");
  }
   ...
   */
  ret = libusb_bulk_transfer(ch347_data->handle, READ_EP, buff, sizeof(buff),
                             NULL, 1000);
  if (ret < 0) {
...
#pragma GCC diagnostic ignored "-Wcast-function-type"
  if (uhModule == 0) {
    ...
    if (uhModule) {
      CH347OpenDevice =
          (pCH347OpenDevice)GetProcAddress(uhModule, "CH347OpenDevice");
      CH347CloseDevice =
          (pCH347CloseDevice)GetProcAddress(uhModule, "CH347CloseDevice");
      CH347ReadData = (pCH347ReadData)GetProcAddress(uhModule, "CH347ReadData");
      CH347WriteData =
          (pCH347WriteData)GetProcAddress(uhModule, "CH347WriteData");
      CH347SetTimeout =
          (pCH347SetTimeout)GetProcAddress(uhModule, "CH347SetTimeout");
